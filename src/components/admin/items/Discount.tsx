...
const Discount: React.FC<DiscountProps> = ({discount, handleInfoClick, handleDeleteClick}) => {
  return (
    <div
      className="discount grid grid-cols-8 h-[45px] px-8 text-gray items-center hover:text-red"
      style={{ gridTemplateColumns: "0.4fr 0.45fr 0.45fr 0.45fr 0.6fr 0.6fr 0.6fr 0.85fr 0.6fr" }}
    >
      ...
      <p className="text-sm font-normal truncate pr-6">{formatDateOnly(discount.expiry_date)}</p>
      <p className="text-sm font-normal truncate pr-6">{discount.movie_id || 'All'}</p>
